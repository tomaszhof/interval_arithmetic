...
template<typename T>
PoissonSolverAM<T>::PoissonSolverAM() {
...
template<typename T>
int PoissonSolverAM<T>::SetExample(int eid) {
	switch (eid) {
	case 1:
		//bc = new Example01();
		break;
...
template<typename T>
int PoissonSolverAM<T>::SolveDIA() {
	if (!_initparams)
		throw runtime_error("Parameters not initialized!");
	int st = 0;
	int n = params.n;
...
template class PoissonSolverAM<mpreal> ;
